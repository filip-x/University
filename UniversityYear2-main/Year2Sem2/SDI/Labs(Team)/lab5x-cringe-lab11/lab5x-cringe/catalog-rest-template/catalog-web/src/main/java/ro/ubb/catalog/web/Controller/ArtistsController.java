...
public class ArtistsController {
    ...
    @RequestMapping(value = "/artists/{id}/instrument/{instrumentId}", method = RequestMethod.POST)
    public ResponseEntity<?> addInstrument(@PathVariable Integer id, @PathVariable Integer instrumentId) throws Exception {
        controller.giveInstrumentToArtist(id, instrumentId);
        return new ResponseEntity<>(HttpStatus.OK);
    }

    @RequestMapping(value = "/artists/{id}/instrument")
    public InstrumentDto getInstrument(@PathVariable Integer id) throws Exception {
        return instrumentConverter.convertModelToDto(controller.getArtist(id).getInstrument());
    }
