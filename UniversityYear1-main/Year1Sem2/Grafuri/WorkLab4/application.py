...
def topological_sort_DFS(graph, vertex, sorted_list, fully_processed_set, in_process_set): # Complexity N since it's DFS and goes through vertices
    #print("\tVertex: " + str(vertex) + "\n\tSorted List: " + str(sorted_list) + "\n" + "\tFully processed set: " +
     #     str(fully_processed_set) + "\n" + "\tIn process set: " + str(in_process_set) + "\n\t-------------")

    in_process_set.add(vertex)
    for adjacent_vertex in graph.get_in_vertices(vertex):
        if adjacent_vertex in in_process_set:
            return False
        elif adjacent_vertex not in fully_processed_set:
            ok = topological_sort_DFS(graph, adjacent_vertex, sorted_list, fully_processed_set, in_process_set)
            if not ok:
               return False
    in_process_set.remove(vertex)
    sorted_list.append(vertex)
    fully_processed_set.add(vertex)
    return True

def topological_sort(graph): # complexity E since it goes through the edges 
    sorted_list = []
    fully_processed_set = set()
    in_process_set = set()
    for vertex in graph.get_vertices():
       if vertex not in fully_processed_set:
            #print("Vertex: " + str(vertex) + "\nSorted List: " + str(sorted_list) + "\n" + "Fully processed set: " +
               #  str(fully_processed_set) + "\n" + "In process set: " + str(in_process_set) + "\n-------------")
            ok = topological_sort_DFS(graph, vertex, sorted_list, fully_processed_set, in_process_set)
            print(ok)
            if not ok:
                sorted_list = None
                break
...
def run_exercise(graph):
    ...
    y = int(input("Enter end vertex: "))
    #print(topo)
    distance = [INFINITY] * len(topo)
    previous = {}
    distance[x] = 0
    previous[x] = None

    for vertex in topo: # Complexity E since it goes through the edges
        for adjacent_vertex in graph.get_out_vertices(vertex):
            if distance[adjacent_vertex] > distance[vertex] - graph.get_cost(vertex, adjacent_vertex):
                distance[adjacent_vertex] = distance[vertex] - graph.get_cost(vertex, adjacent_vertex)
                previous[adjacent_vertex] = vertex
    path = get_path(x, y, previous)
    print(path)
    
def get_path(start, stop, previous): # worst case complexity N
    path = [stop]
    while stop != start:
        stop = previous[stop]
        path.append(stop)
