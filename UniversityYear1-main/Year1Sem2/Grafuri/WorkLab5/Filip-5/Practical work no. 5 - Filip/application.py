...
class Graph:
   ...
   # The give edge will leave from the vertix x and will go into vertix y
    def add_edge(self, x, y, price):
        ...
        # if the edge that leaves x is not connected to y yet then it will be connected
        if (y in self.out_vertices[x]) == False:
            self.out_vertices[x].append(y)
        # if the edge that goes into y is not connected to x then it will be connected
        if (x in self.in_vertices[y]) == False:
    ...
    def delete_edge(self, x, y):
        if (x in self.out_vertices and y in self.in_vertices and y in self.out_vertices[x] and x in self.in_vertices[y]) == False:
            raise Exception(
                "Error Occured: One of the vertices does not exist or the vertices are not connected!")
        # removes the edge when the edge leaves the vertix x
        self.out_vertices[x].remove(y)
        # removes the edge when the edge  goes into the vertix y
    ...
    # degree of the edges that go out of the vertex
    def get_out_degree_of_vertex(self, vertex):
        if (vertex in self.out_vertices) == False:
            raise Exception("Error Occured: The vertex doesn't exist!")
        # it returns the length of the vertex that go out of the vertices we want
        return len(self.out_vertices[vertex])

    # list of vertices that have a edge from the vertix as parameter
    def get_out_vertices(self, vertex):
        if (vertex in self.out_vertices) == False:
            raise Exception("Error Occured: The vertex doesn't exist!")
        return self.out_vertices[vertex][:]

    # list of vertices that have a edge that goes into the vertex as parameter
    ...
    def price_change(self, x, y, new_price):  # change of the "cost"
        if (x in self.out_vertices and y in self.in_vertices and y in self.out_vertices[x]) == False:
            raise Exception(
                "Error Occured: The vertices don't exist or aren't connected together!")
...
def read_graph_from_file():  # if we have a pragh in the file we enter the file name in the console and it will give read  the graph from the file
    ...
    float(edge_number)  # to get rid of warning
...
# display the number of vertice of our graph
...
def valid_edge(my_graph):
    ...
    if my_graph.valid_edge(x, y) == True:
        print("There exists the ({0}, {1})={2} edge".format(
            x, y, my_graph.price[x][y]))
    else:
        print(
            "Error Occured:There doesn't exist an edge from {0} to {1} ".format(x, y))


def get_in_degree(my_graph):
    vertex = int(input("Vertex: "))
    print("The vertex {0} has an indegree of {1}".format(
        vertex, my_graph.get_in_degree_of_vertex(vertex)))


def get_out_degree(my_graph):
    vertex = int(input("Vertex: "))
    print("The vertex {0} has an outdegree of {1}".format(
        vertex, my_graph.get_out_degree_of_vertex(vertex)))
...
def file_graph_saver(my_graph):
    ...
    file_open.write("{0} {1}\n".format(
        my_graph.vertex_number, my_graph.edge_number))
...
#main function
def minimum_cost_path(some_graph, level, vertex, S, start):
    
    minimum = INFINITY
    path_so_far = []
    if level > 0:
        S.remove(vertex)
        for y in some_graph.get_in_vertices(vertex):
            if y in S:
                previous_cost, path = minimum_cost_path(some_graph, level - 1, y, S, start)# for the neighbor
                cost = some_graph.get_price(y, vertex) + previous_cost
                if cost < minimum:# to get the minimum path
                    minimum = cost
                    path_so_far = path
        S.append(vertex)
    elif vertex == start:
        return 0, [start]
    path_so_far.append(vertex)
    return minimum, path_so_far

# EXERCISE 5
def GetDistance(SomeGraph):
    x = int(input("x = "))#from where -> to where 
    y = int(input("y = "))
    count = len(SomeGraph.get_vertices())
    neighbors = SomeGraph.get_vertices()
    minimum_cost = INFINITY
    path = []
    for i in range(count):#for every lenght possible it stors the minimum path 
        level_cost, temp_path = minimum_cost_path(SomeGraph, i, y, neighbors, x)#here we call the minimum cost path
        #print("")
        if level_cost < minimum_cost:
            minimum_cost = level_cost
            path = temp_path
    if len(path) == 0:
        print("There's no path from {0} to {1}.".format(x, y))
    else:
        print("The walk cost from {0} to {1} is {2}".format(x, y, minimum_cost))
        print(path)
...
def display_menu():
    ...
    print("13. Copy graph")
    # it can without creating the file
