...
void Ui::start()
{
	...
	string current_mode="";
	Repo_my_list* my_list = NULL;
	Repository_Inherit* repository_saver = NULL;
	while (true)
	{
		try
		{
			...
			else if ( words_from_command == "fileLocation")
			{
				...
				std::string path;
				getline(reader, path);
				repository_saver = new File_Repository(path);
				if (repository_saver != NULL && my_list != NULL)
					service = new Service(repository_saver, my_list);
			}
			else if (words_from_command == "mylistLocation")
			{
				reader.get();
				std::string path;
				getline(reader, path);
				if (path.substr(path.rfind('.') + 1, 3) == "csv")
					my_list = new My_list_CSV(path);
				else if (path.substr(path.rfind('.') + 1, 3) == "htm")
					my_list = new My_list_html(path);
				if (repository_saver != NULL && my_list != NULL)
					service = new Service(repository_saver, my_list);
			}
			else if (words_from_command == "add" && (current_mode == "mode A"))// ADD
			{
				...
				else
				{
					...
					if (Validation::validation_int(access_count_command) == 0)
					...
					if (Validation::validation_date(date_of_creation) == 0)
			...
			else if (words_from_command == "open" && (current_mode == "mode B"))// to do 
			{
				service->open_my_list();
			}
			else
			{
