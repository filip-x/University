...
class RepoGradeJsonFile(RepoGradeFile):

    def read_repo(self):
        try:
            open_file = open(self.file_n, "r")
            save_grades_lists = json.load(open_file)
            for grade_list in save_grades_lists:
                # we created a grade
                file_grade = Grade(grade_list[0], grade_list[1], grade_list[2])
    ...
    def write_repo(self):
        ...
        for i in range(len(self.grades)):
            ...
            if self.grades[i] != None:
                grade_list = [self.grades[i].get_assignment(
                ), self.grades[i].get_student(), self.grades[i].get_grade()]
                save_grades_lists.append(grade_list)
        json.dump(save_grades_lists, open_file)
        open_file.close()
